...
fn legacy_cert_pbes2_key() {
    ...
    let p12_bytes =
        openssl_export_legacy_pbe(&cert_pem, &key_pem, "AES-256-CBC", "PBE-SHA1-3DES");
...
fn pbes2_cert_legacy_key() {
    ...
    let p12_bytes =
        openssl_export_legacy_pbe(&cert_pem, &key_pem, "PBE-SHA1-3DES", "AES-256-CBC");
...
fn pkcs12_kdf_known_answer() {
    ...
    let key = derive_key_utf8::<Sha1>(password, &salt, Pkcs12KeyType::EncryptionKey, iterations, key_len)
        .expect("KDF should succeed");
    assert_eq!(key.len(), key_len);

    // Derive again with same inputs — must be deterministic
    let key2 = derive_key_utf8::<Sha1>(password, &salt, Pkcs12KeyType::EncryptionKey, iterations, key_len)
        .expect("KDF should succeed");
    assert_eq!(key, key2);
...
fn pkcs12_3des_cbc_round_trip() {
    ...
    let key = derive_key_utf8::<Sha1>(password, &salt, Pkcs12KeyType::EncryptionKey, iterations, 24)
...
fn pkcs12_rc2_40_cbc_round_trip() {
    ...
    buf[..plaintext.len()].copy_from_slice(plaintext);
    let ciphertext =
        cbc::Encryptor::<rc2::Rc2>::inner_iv_slice_init(enc_cipher, &iv)
            .expect("encryptor init")
            .encrypt_padded::<Pkcs7>(&mut buf, plaintext.len())
            .expect("encrypt");
    let ct_vec = ciphertext.to_vec();
    ...
    let mut dec_buf = ct_vec.clone();
    let decrypted =
        cbc::Decryptor::<rc2::Rc2>::inner_iv_slice_init(dec_cipher, &iv)
            .expect("decryptor init")
            .decrypt_padded::<Pkcs7>(&mut dec_buf)
            .expect("decrypt");
...
fn process_p12_test() {
    ...
    let password = "////////";
    assert!(pkcs12_builder::get_key_and_cert(&enc_p12, password).is_ok())
}
